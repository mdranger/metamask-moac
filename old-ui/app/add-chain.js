...
module.exports = connect(mapStateToProps)(AddChainScreen)
...
inherits(AddChainScreen, Component)
...
AddChainScreen.prototype.render = function () {
  const state = this.state
  const props = this.props
  const { warning, symbol, monitor } = state
      ...
      h('.flex-column.flex-justify-center.flex-grow.select-none', [
        h('.flex-space-around', {
          ...
          h('div', [
            h(Tooltip, {
              position: 'top',
              title: 'The address of the MicroChain. Click for more info.',
            }, [
              h('a', {
                style: { fontWeight: 'bold', paddingRight: '10px'},
                href: 'http://explorer.moac.io/mclist',
                target: '_blank',
              }, [
...
AddChainScreen.prototype.tokenAddressDidChange = function (event) {
...
AddChainScreen.prototype.validateInputs = function () {
  let msg = ''
  const state = this.state
  ...
  if (ownAddress) {
    msg = 'Personal address detected. Input the MicrChain address.'
  }
...
AddChainScreen.prototype.attemptToAutoFillTokenParams = async function (address) {
  ...
  if (symbol && decimals) {
    console.log('SETTING Name AND DECIMALS', { symbol, decimals })
